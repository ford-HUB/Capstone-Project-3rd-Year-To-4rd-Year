...
const EventCalendar = ({ eventData }) => {
    ...
    const events = React.useMemo(() => {
        ...
        return eventData.reduce((acc, event) => {
            const dateObj = dayjs(event.time);
            if (
                    ...
                    <div
                        className={`w-10 h-10 flex items-center justify-center text-sm cursor-pointer rounded-lg transition-all duration-200 relative ${
                            isToday
                                ...
                                : hasEvents
                                ? 'bg-gradient-to-br from-blue-50 to-purple-50 text-gray-800 font-medium hover:from-blue-100 hover:to-purple-100 border border-blue-200'
        ...
        <div className="bg-white border-gray-200 p-4 max-w-sm">
            ...
            <div className="border-t border-gray-200 pt-3">
                ...
                <div className="space-y-2">
                    {[
                        ...new Map(
                            eventData.map((item) => [item.type, item])
                        ).values(),
                    ].map((event) => {
                        const colors = getEventCategoryColors(event.type);
                        return (
                            <div
                                key={event.event_id}
                                ...
                                <div className={`text-xs ${colors.text} opacity-70`}>
                                    {eventData.filter(e => e.type === event.type).length}
                                </div>
                            </div>
                        );
                    })}

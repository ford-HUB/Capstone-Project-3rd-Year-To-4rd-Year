...
import { getPaymentDisplayName } from '../../utils/paymentUtils';
    ...
    <div className="fixed inset-0 bg-black/20 backdrop-blur-[2px] flex items-center justify-center z-50">
      <div className="bg-white rounded-lg p-6 max-w-md w-full mx-4">
          ...
          {selectedPayment && (
            <div className="bg-gray-50 p-3 rounded-lg">
              <p className="text-sm text-gray-700">
                <span className="font-medium">Payment Method:</span> {getPaymentDisplayName(selectedPayment.payment_method_types?.[0])}
              </p>
              <p className="text-sm text-gray-700">
                <span className="font-medium">Current Status:</span> {selectedPayment.status || 'Pending'}
              </p>
        ...
        <div className="flex justify-end space-x-3">
          ...
          <button
            onClick={() => {
              console.log('Continue to Update button clicked');
              onConfirm();
            }}

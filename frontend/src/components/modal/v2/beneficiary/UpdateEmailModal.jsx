...
import { beneficiaryEmailUpdateSchema } from '../../../../forms/BeneficiarySchemas.js';
...
const UpdateEmailModal = ({ isOpen, setOpen, data }) => {
    const { updateEmail } = useBeneficiaryProfileStore()
    const navigate = useNavigate()
    const { handleSubmit, register, formState: { errors, isSubmitting, isValid } } = useForm({
        resolver: zodResolver(beneficiaryEmailUpdateSchema),
        defaultValues: {
            newEmail: '',
            confirmEmail: ''
        }
    ...
    console.log('Form state - isSubmitting:', isSubmitting, 'isValid:', isValid, 'errors:', errors)

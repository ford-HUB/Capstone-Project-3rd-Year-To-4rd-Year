...
import DirectorSidePanel from '../../components/director/DirectorSidebarPanel.jsx';
...
const ManageApprovals = () => {
  const { requestsList, getRequestList, approve, reject, deleteRequest } = useApprovalStore();
  ...
  }, [getRequestList, requestsList.length]);
  ...
  const handleApproval = async (requestId, newStatus) => {
    ...
    switch (newStatus) {
      ...
      case 'rejected':
        success = await reject(requestId);
        break;
